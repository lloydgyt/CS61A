...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        while opponent_score > 0:
            ...
            func = digit_fn(rightmost_digit)
            points = func(points) # cycle through all function
            opponent_score //= 10
    return points % 30
...
def hog_pile(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    rdigit_p = player_score % 10
    rdigit_o = opponent_score % 10
    if rdigit_o == rdigit_p:
        return rdigit_p
    else:
        return 0
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0: # player0's turn
            num_rolls = strategy0(score0, score1)
            score0 += take_turn(num_rolls, score0, score1, dice=dice, goal=goal)
            score0 += hog_pile(score0, score1)
        else:
            num_rolls = strategy1(score1, score0)
            score1 += take_turn(num_rolls, score1, score0, dice=dice, goal=goal)
            score1 += hog_pile(score1, score0)
        who = (who + 1) % 2 # alternate turns
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        current_leader = None
    # decide message return
    if current_leader == None or current_leader == last_leader:
        message = None
    else:
        dist = abs(score0 - score1)
        message = f"Player {current_leader} takes the lead by {dist}"
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, current_highest_score = 1, 0
    i = 1
    while i <= 10: 
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_by_rolling_0 = hefty_hogs(score, opponent_score)
    if hog_pile(score + score_by_rolling_0, opponent_score) > 0:
        return 0
    elif score_by_rolling_0 >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
