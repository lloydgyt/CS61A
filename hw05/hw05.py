def chain(t):
    ...
    """
    def chain_k(tree, k):
        """ check if there exist a chain of label k"""
        if tree.is_leaf():
            return tree.label == k
        if all([not chain_k(b, k) for b in tree.branches]):
            return False
        else:
            return True
    return chain_k(t, t.label)
...
def flatten_link(lnk):
    ...
    if isinstance(lnk.first, Link):
        list.extend(flatten_link(lnk.first))
    else:
        list.append(lnk.first)
    list.extend(flatten_link(lnk.rest))
    return list
...
def has_path(t, term):
    ...
    if t.label != term[0]:
        ...
        if len(term) == 1:
            return True
        if not all([not has_path(b, term[1:]) for b in t.branches]):
            return True
    return False
...
def duplicate_link(lnk, val):
    ...
    while curr != Link.empty:
        if curr.first == val: # find the target
            dup_node.rest, curr.rest = curr.rest, dup_node
            break
        curr, prev = curr.rest, curr
